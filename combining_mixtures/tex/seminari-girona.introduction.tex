...
\item For each $\obs{\textbf{x}_i}$ a vector of probabilities/weights $\post{\tau_{i1}}, \dots, \post{\tau_{ik}}$ giving the relative chances to belong to certain classes $C_1, \dots, C_k$
\end{itemize}
}
\only<6->{
\begin{itemize}
\item<6-> A sample $\post{\boldsymbol\tau_1}, \dots, \post{\boldsymbol\tau_n}$
\item<7> An implicit set classes\\ $\;\;C_1, \dots, C_k$
\end{itemize}
}
\bigskip
\begin{center}
