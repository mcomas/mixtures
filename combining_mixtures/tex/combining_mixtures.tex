...
\[
\hat{\tau}_{i I_j} =  \frac{ \hat{\pi}_{I_j} \hat{f}_{I_j}(\m x_i) }{\sum_{i=1}^s \hat{\pi}_{I_j} \hat{f}_{I_j}(\m x_i)},
\]
...
In this article, we are not interested in how to decide which of the different partitions in a hierarchical combination of components defines a ``better'' clustering, but to propose different methods to decide how to merge the components. More concretely, given a finite mixture we are interested in analysing methods to decide how to merge components only using only the information given by the posterior probabilities $\hat{\tau}_{i \mathcal{I}}$, $1\leq i \leq n$.
...
\sum_{i=1}^n \hat{\tau}_{i I_a \cup I_b} \log(\hat{\tau}_{i I_a \cup I_b}) - \hat{\tau}_{iI_a} \log(\hat{\tau}_{iI_a}) - \hat{\tau}_{iI_b} \log(\hat{\tau}_{iI_b}).
\]
respect to $I_a$ and $I_b$ is equivalent to minimize
\[
...
\]
and it is equivalent to minimize
\[
\frac{\sum_{i=1}^n c \; ( \hat{\tau}_{iI_a} \log(\hat{\tau}_{iI_a}) + \hat{\tau}_{iI_b} \log(\hat{\tau}_{iI_b}) - \hat{\tau}_{i I_a \cup I_b} \log(\hat{\tau}_{i I_a \cup I_b}) )}{\sum_{i=1}^n c}
\]

%The only difference between the defined function $\lambda(\hat{\tau}_{i \mathcal{I}_s}, I_a, I_b)$ and the Total Entropy is that the first is a minimisation problem and the second is a maximisation problem. Therefore, multiplying by -1 results into the same optimisation problem.
\end{proof}
