...
if(!exists('lambda')) lambda = 'DEMP'
if(!exists('omega')) omega = 'prop'
if(!exists('OFILE')) OFILE = sprintf('data/hp_sim-mo_01-10-spherical-%s-%s-fig02.RData', lambda, omega)
...
HP = llply(simulation, function(sim){
  tm = system.time(hp <- get_hierarchical_partition(tau = sim$tau, omega = f_omega, lambda = f_lambda))
  list(
    'time' = tm,
