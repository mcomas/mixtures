...
load('data/coda_skew_t_mixture_c.RData')
...
df.ilr = data.frame(ilr(X, V = B))
names(df.ilr) = c('coord.1', 'coord.2')
...
p.ilr = p.ilr +
  xlab( expression(paste(ilr[1]))) +
  ylab( expression(paste(ilr[2])))
...
l_ply(cl, function(v){
  df.c = data.frame(ilrInv(data.frame(v$x,v$y), V = B))
...
pdf(file='figures/coda_skew_t_mixture_c.pdf', width=17, height=10, pointsize=25)
grid.newpage()
